    ...
    <div id="search" :class="{'searching': isShow}" @click="isShow && $eventBus.emit('global-search-toggle')">
        <div class="container">
            <div class="search-box" @click.stop>
                <el-input ref="input" v-model="searchInput" :placeholder="t('app.search.input')" clearable @keydown.esc="$eventBus.emit('global-search-toggle')" @keydown.up.prevent="keyUp" @keydown.down.prevent="keyDown" @keydown.enter.prevent="keyEnter">
                    <template #prefix>
                        <el-icon>
                            <i-ep-search />
                        </el-icon>
...
onMounted(() => {
    ...
    proxy.$hotkeys('alt+s', e => {
        if (settingsStore.topbar.enableNavSearch && windowStore.list.every(item => !item.isMaximize)) {
            e.preventDefault()
            isShow.value = true
        }
    })
